...
class Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        # weight(tensor).data (only copy data from tensor)
        ...
        self.char_cnn = CharacterLevelCNN(config.char_dim, config.char_hidden, 5)
...
class CharacterLevelCNN(nn.Module):
    ...
    def __init__(self, char_dim, char_hidden, num):
        super(CharacterLevelCNN, self).__init__()
        self.conv1 = nn.Sequential(nn.Conv1d(char_dim, char_dim, kernel_size=3, padding=1), nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv1d(char_dim, char_dim, kernel_size=3, padding=1), nn.ReLU())
        self.conv3 = nn.Sequential(nn.Conv1d(char_dim, char_hidden, kernel_size=3, padding=1), nn.ReLU())
        self.conv4 = nn.Sequential(nn.Conv1d(char_hidden, char_hidden, kernel_size=3, padding=0), nn.ReLU(),
                                   nn.MaxPool1d(3))
